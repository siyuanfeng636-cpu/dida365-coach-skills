...
2. 如果是首次接入或连接失败，默认先读取 [`references/mcporter-backend-setup.md`](./references/mcporter-backend-setup.md) 走 MCPorter + 远程 MCP 接入层；如果当前环境没有 MCPorter，再读取 [`references/mcp-client-setup.md`](./references/mcp-client-setup.md) 给出远程 MCP 最短路径；只有用户明确想走“像 Getnote 一样的本地授权”时，才读取 [`references/openapi-auth-setup.md`](./references/openapi-auth-setup.md)。
...
- `references/mcporter-backend-setup.md`
  用于把 dida 接入/OAuth 层交给 MCPorter 管理，减少手工配置和外部 MCP 接入失败率。
...
- `tools/mcporter_backend.py`
  用于生成 MCPorter backend 安装命令和 manifest，把 dida 接入层包装成可由 MCPorter 管理的本地 backend。
...
- 默认优先走 “MCPorter + 远程 MCP” 路线：先注册或调用 `dida-auth-backend`，再由 backend 写入 OpenClaw 的 dida365 HTTP MCP 配置，并引导点击一次连接按钮完成浏览器 OAuth。
...
- OpenClaw 的兜底路径才是“半自动接入”：先自动写 `~/.openclaw/openclaw.json`，再让用户在 MCP / Tools / 依赖面板里点 `Connect`、`Authorize` 或 `Sign in`。
- 如果 MCPorter 可用，默认把 dida 接入/OAuth 放到 `dida-auth-backend` 里，由 backend 处理状态检测、OpenClaw 配置写入和远程 MCP 授权；本地 Open API OAuth 只作为用户明确要求时的备选。
