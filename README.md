...
大多数 AI 效率工具止步于创建任务。Dida-Coach 更进一步：它能规划、排程、追踪、复盘，并从你的执行数据中学习——全程通过自然对话完成。
...
AI 方案停在聊天窗口，永远到不了手机上



通过 MCP 协议同步至滴答清单——手机、平板、手表随时可用
...
智能工作方法引擎根据任务类型与精力状态推荐 25 / 30 / 50 / 90 分钟时段
...
每个时间盒包含一个可验证的交付物——而非仅仅是一段时长。引擎自动推荐最佳工作方法：
...
每次写入操作均遵循写入-读取-验证协议——教练会从滴答清单回读数据，确认字段已正确持久化。
...
严格的路由表将每一种用户意图映射到真实的滴答清单 MCP 工具名——杜绝幻觉式 API 调用。
...
错过的任务会先进行根因分析（难度过高？注意力分散？时段不对？），再决定重排方案——而非简单地"明天再试"。
...
其他客户端请参阅 references/mcp-client-setup.md。
...
作者：@siyuanfeng636-cpu
OpenClaw 生态：github.com/openclaw
协议：MIT
